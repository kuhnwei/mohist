...
public class FileUploadCommand extends Command implements IPostCommand {
	...
	public void execute(final OutputStream out) throws ConnectorException {
		...
		try {
			...
			if (this.responseType != null && this.responseType.equals("txt")) {
				...
				System.out.println("--------- Path:[" + path + "] ----------- newFileName:[" + this.newFileName + "]---------------");
			} else {
				if (checkFuncNum()) {
	...
	private boolean saveTemporaryFile(final String path, final FileItem item)
		...
		String pathName = configuration.getTypes().get(type).getUrl() + currentFolder;
		if (!ImageUtils.isImage(file)) {
			// 文件类型不是图片
			item.write(file);
			// TODO 上传FTP
			// CkfinderFtpClient.uploadFile(item.getInputStream(), pathName, this.fileName);
	...
	protected boolean checkConnector(final HttpServletRequest request) throws ConnectorException {
		System.out.println("--------------------- FileUploadCommand#checkConnector 文件上传时权限认证 ---------------------------");
