...
public class ConnectorServlet extends HttpServlet {
    ...
    private void getResponse(final HttpServletRequest request, final HttpServletResponse response, final boolean post) throws ServletException {
        request.getSession().setAttribute("CKFinder_UserRole", "view_files");
...
//            if (initCommand.equalsIgnoreCase(command)) {
//                CkfinderFtpClient.download(request, configuration.getBaseURL());
//            }
        } catch (Exception e) {
        ...
        try {
            ...
            if (CommandHandlerEnum.contains(command.toUpperCase())) {
                ...
                // 检查命令是否应该通过POST请求，或者是POST请求
                // and it's not upload command
                // 而且它不是上传命令
                boolean isPostAndNotUpload = (cmd.getCommand() instanceof IPostCommand || post)
                        && !CommandHandlerEnum.FILEUPLOAD.equals(cmd)
                        && !CommandHandlerEnum.QUICKUPLOAD.equals(cmd);
                if (isPostAndNotUpload) {
